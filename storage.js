...
function createDatabase(filePath = path.join(__dirname, 'kingbot.sqlite')) {
  ...
  return {
    ...
    async createBot(userId) {
      ...
      db.prepare('INSERT INTO bots (id, userId, name, strategy, timeframe, status, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)').run(id, userId, name, 'Mean Reversion', '1h', 'paused', createdAt);
      return { id, userId, name, strategy: 'Mean Reversion', timeframe: '1h', status: 'paused', createdAt };
    },
